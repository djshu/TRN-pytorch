...
def return_something(modality):
    ...
    # filename_categories = os.path.join(p_d_root_path,"something2something",'category.txt')
    if modality == 'RGB':


        # p_d_root_path = "/data/sjd/d/p_d/TRN-pytorch"
        root_data = '/data/sjd/d/video/something-something-v1/unzip/20bn-something-something-v1'
...
def return_charades(modality):
    ...
    if modality == 'RGB':
        ...
        root_data = '/data/sjd/d/video/charades/Charades_v1_rgb'
    else:
        print('no such modality:'+modality)
        os.exit()
    return filename_categories, filename_imglist_train, filename_imglist_val, root_data, prefix
...
def return_dataset(dataset, modality):
    ...
    dict_single = {'jester': return_jester, 'something-something-v1': return_something, 'somethingv2': return_somethingv2,
                   'charades': return_charades, 'moments': return_moments}
    ...
    file_imglist_train = os.path.join(p_d_root_path, file_imglist_train)
    file_imglist_val = os.path.join(p_d_root_path, file_imglist_val)
    file_categories = os.path.join(p_d_root_path, file_categories)
